...
for (rep in 1:restarts) {
	for (n in seq(500,5000,500)) {	
		for (k in seq(100,1000,100)) {	
            ...
            g_x    = sum(sim) / M
			in.eval = (pi / 2) * g_x
			diff   = (abs(in.eval - eval_wolfram) / eval_wolfram) * 100
...
p1 <- 	wireframe(time ~ n * k, data = result,
		  ...
		  main          = "Tiempo de ejecución de la simulación",
		  drape         = TRUE,
		  colorkey      = FALSE,
...
p2 <- 	wireframe(diff ~ n * k, data = result,
		  ...
		  main          = "Desviación respecto el valor de Wolfram",
		  drape         = TRUE,
		  colorkey      = FALSE,
		  scales        = list(arrows=FALSE), # z = list(arrows=TRUE)),

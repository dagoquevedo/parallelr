...
for(N in c(50,100,1000,5000,10000)) {
	...
	ac <- data.frame()
	ac <- rbind(ac, cbind(metodo = "Método 1", val = round(ac1[2] * 100), accuracy = "WMAPE"))
	ac <- rbind(ac, cbind(metodo = "Método 2", val = round(ac2[2] * 100), accuracy = "WMAPE"))
	
	ac$val = as.double(levels(ac$val))[ac$val]
	
	df <- data.frame()
	df <- rbind(df, data.frame(x = rep(1:M), y = data.exp$valor		  , Variable = "Real"))
	df <- rbind(df, data.frame(x = rep(1:M), y = result.method.1$valor, Variable = "Método 1"))
	...
	plot2 = ggplot(data =ac , aes(x = metodo, y = val, fill = as.factor(accuracy)))+
			...
			xlab("Método")+ylab("Valor de evaluación (%)") +
